...
// Funcion KNN Añadir Nueva Lista

void compararKNNyAnadirANuevaLista (tipoLista* listaAdevolver,tipoLista *lista,int mejork)
{
    celdaLista* aComparar;
    celdaLista* recorrido;

    aComparar = lista -> ini;
    int Vivoscaballos, CaballosComparar;
    ...
    while (aComparar != lista -> fin)
    {
        ...
        CaballosComparar = aComparar -> elem.frecuenciaRespiracion;
        recorrido = lista -> ini;
        ...
        while(recorrido != lista -> fin)
        {
            if(recorrido != aComparar)
            {
                ...
                if(devolverRaiz(mon).distancia > distancia2)
                {
                    ...
                    tupla.saludCaballo = recorrido->elem.frecuenciaRespiracion;
                    insertarMaxMonticulo(&mon, tupla);
                }
            }
            recorrido = recorrido -> sig;
        }
        compFinal cantidades[75]; //Array de 75 huecos en la que guardaremos la cantidad que acierta de cada tipo(entre 0/75) y la distancia mÃ­nima a travÃ©s del struct compFinal

        for(int j = 0; j < 75; j++)
        {
            cantidades[j].cant = 0;
            cantidades[j].distMin = MAX;
        }
        ...
        for (i = 0; i < mon.pos; i++)
        {
            for (j = 0; j < 75; j++)
            {
                if (mon.array[i].saludCaballo == (j+1))
                {
                    cantidades[j].cant++;

                    if (mon.array[i].distancia < cantidades[j].distMin)
                    {
                        cantidades[j].distMin = mon.array[i].distancia;
                    }
                }
            }
        }
        ...
        for(i = 1; i < 75; i++)
        {
            if (cantidades[Vivoscaballos].cant < cantidades[i].cant)
            {
                Vivoscaballos = i;
            }
            else if (cantidades[Vivoscaballos].cant == cantidades[i].cant)
            {
                if (cantidades[Vivoscaballos].distMin > cantidades[i].distMin)
                {
                    Vivoscaballos = i;
                }
            }
        ...
        Vivoscaballos++;
        if(Vivoscaballos == CaballosComparar)
        {
            total++;
            acertados++;
